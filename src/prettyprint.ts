...
export function prettyKind(k: Kind): string {
  switch (k.tag) {
    case KindTag.Star: return "*";
    case KindTag.Arrow: return `(${prettyKind(k.from)} -> ${prettyKind(k.to)})`;
    case KindTag.Constraint: return "Constraint";
  }
}
...
function prettyCtor(ctor: GADTConstructor): string {
  const existentials = ctor.existentials.length > 0
    ? `∀${ctor.existentials.map((e) => e.variable.name).join(" ")}. `
    : "";
  const constraints = ctor.constraints.length > 0
...
export function prettyCoreExpr(expr: CoreExpr, indent = 0): string {
  ...
  switch (expr.tag) {
    ...
    case "CoreCase":
      const alts = expr.alternatives.map((a) => prettyCoreAlt(a, indent + 1)).join("\n");
...
function prettyCoreAlt(alt: CoreAlt, indent: number): string {
  const pad = "  ".repeat(indent);
  const exVars = alt.existentials.length > 0
    ? ` ∃${alt.existentials.map((e) => e.var.name).join(" ")}.`
    : "";
  const coercions = alt.coercions.length > 0
    ? ` [${alt.coercions.map(prettyCoercion).join(", ")}]`
    : "";
  const binds = alt.bindings.map((b) => `${b.name}:${prettyType(b.type)}`).join(", ");
...
export function prettyCoercion(co: Coercion): string {
  switch (co.tag) {
    case "CoRefl": return `refl(${prettyType(co.type)})`;
    case "CoSym": return `sym(${prettyCoercion(co.coercion)})`;
    case "CoTrans": return `trans(${prettyCoercion(co.first)}, ${prettyCoercion(co.second)})`;
    case "CoArrow": return `arrow(${prettyCoercion(co.param)}, ${prettyCoercion(co.result)})`;
    case "CoApp": return `app(${prettyCoercion(co.constructor)}, ${prettyCoercion(co.argument)})`;
    case "CoForall": return `forall(${co.variable.name}, ${prettyCoercion(co.body)})`;
    case "CoAxiom": return `axiom(${co.name}: ${prettyType(co.lhs)} ~ ${prettyType(co.rhs)})`;
    case "CoVar": return `co_${co.name}`;
  }
}
