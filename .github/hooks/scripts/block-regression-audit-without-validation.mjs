...
async function main() {
  ...
  if (!raw.trim()) {
    process.stdout.write(preToolAllow("No payload; skipping regression validation gate."));
    return;
  }
  ...
  try {
    ...
    process.stdout.write(preToolAllow("Invalid payload JSON; skipping regression validation gate."));
    return;
  }

  const toolName = getToolName(payload);
  if (toolName !== "task_complete") {
    process.stdout.write(preToolAllow("Not a completion step; skipping regression validation gate."));
    return;
  }
  ...
  if (hasValidationEvidence) {
    process.stdout.write(preToolAllow("Regression-audit completion includes validation evidence; allowing."));
    return;
  }
